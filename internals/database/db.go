...
func InitDB() error {
	// The params in the DSN run on every pooled connection (a plain PRAGMA
	...
	// requests write at the same time. These are the names the go-sqlite3
	// driver understands (the "_pragma=" form is silently ignored by it).
	var err error
	DB, err = sql.Open("sqlite3", "./forum.db?_foreign_keys=1&_busy_timeout=5000")
	if err != nil {
